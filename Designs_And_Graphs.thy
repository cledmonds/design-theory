...
lemma arc_to_ends_to_block: "arc_to_block e = {x, y} \<Longrightarrow> arc_to_ends G e = (x, y) \<or> arc_to_ends G e = (y, x)"
  by (metis arc_to_block_def arc_to_ends_def doubleton_eq_iff)

lemma arc_to_block_sym: "arc_to_ends G e1 = (u, v) \<Longrightarrow> arc_to_ends G e2 = (v, u) \<Longrightarrow> arc_to_block e1 = arc_to_block e2"
...
lemma arc_ends_blocks_subset: "E \<subseteq> arcs G \<Longrightarrow> (x, y) \<in> ((arc_to_ends G) ` E) \<Longrightarrow> {x, y} \<in> (arc_to_block ` E)"
...
proof -
  obtain e where "e \<in> E" and "arc_to_block e = {x,y}" using assms
    by fastforce 
  then have "arc_to_ends G e = (x, y) \<or> arc_to_ends G e = (y, x)" using arc_to_ends_to_block by simp
  thus ?thesis
...
next 
  fix xa
  assume assm1: "{x, y} = arc_to_block  xa" and assm2: "xa \<in> (arcs G)"
  thus "(y, x) \<in> arc_to_ends G ` arcs G"
    by (metis (mono_tags, hide_lams) arc_blocks_end_subset arcs_ends_def graph_symmetric image_iff subset_refl)
qed
...
lemma arcs_ends_blocks_wf: assumes "bl \<in># arcs_blocks" shows "bl \<subseteq> verts G"
proof - 
  ...
  thus ?thesis using arcs_ends_wf
    using blpair by auto
qed

lemma arcs_blocks_simple: assumes "bl \<in># arcs_blocks" shows "count (arcs_blocks) bl = 1"
  apply (simp add: arcs_blocks_def)
  using assms count_mset_set finite_arcs
  by (metis One_nat_def arcs_blocks_def count_eq_zero_iff)
...
lemma in_out_arcs_reflexive: "v \<in> verts G \<Longrightarrow> (e \<in> (in_arcs G v) \<Longrightarrow> \<exists> e' . (e' \<in> (out_arcs G v) \<and> head G e' = tail G e))"
  using symmetric_conv sym_arcs by fastforce 

lemma out_in_arcs_reflexive: "v \<in> verts G \<Longrightarrow> (e \<in> (out_arcs G v) \<Longrightarrow> \<exists> e' . (e' \<in> (in_arcs G v) \<and> tail G e' = head G e))"
  using symmetric_conv sym_arcs by fastforce 
...
lemma in_out_arcs_reflexive_uniq: "v \<in> verts G \<Longrightarrow> (e \<in> (in_arcs G v) \<Longrightarrow> \<exists>! e' . (e' \<in> (out_arcs G v) \<and> head G e' = tail G e))"
  apply auto
    using symmetric_conv apply fastforce
  using out_arcs_single_per_vert by (metis head_in_verts in_out_arcs_conv) 

lemma out_in_arcs_reflexive_uniq: "v \<in> verts G \<Longrightarrow> e \<in> (out_arcs G v) \<Longrightarrow> \<exists>! e' . (e' \<in> (in_arcs G v) \<and> tail G e' = head G e)"
  apply auto
    using symmetric_conv apply fastforce
  using in_arcs_single_per_vert by (metis tail_in_verts in_in_arcs_conv) 

lemma in_eq_out_arc_ends: "(u, v) \<in> ((arc_to_ends G) ` (in_arcs G v)) \<longleftrightarrow> (v, u) \<in> ((arc_to_ends G) ` (out_arcs G v))"
  by (smt (z3) Pair_inject adj_in_verts(1) arc_to_ends_def dominatesI image_iff in_in_arcs_conv in_out_arcs_conv out_in_arcs_reflexive_uniq)
...
lemma bij_betw_in_out_arcs: "bij_betw (\<lambda> (u, v) . (v, u)) ((arc_to_ends G) ` (in_arcs G v)) ((arc_to_ends G) ` (out_arcs G v))"
...
lemma in_eq_out_degree: "in_degree G v = out_degree G v"
  using bij_betw_in_out_arcs bij_betw_same_card in_degree_eq_card_arc_ends out_degree_eq_card_arc_ends
  by auto 
...
proof - 
  have "\<And> e . e \<in> (arc_to_block ` (arcs G)) \<Longrightarrow> x \<in> e \<Longrightarrow> e \<in> (arc_to_block ` in_arcs G x)"
    using arc_to_block_def in_in_arcs_conv insert_commute insert_iff singleton_iff sym_arcs symmetric_conv by fastforce
  then have "{ e \<in> (arc_to_block ` (arcs G)) . x \<in> e} =  (arc_to_block ` (in_arcs G x))" 
    using arc_to_block_def by auto
  then have "card { e \<in> (arc_to_block ` (arcs G)) . x \<in> e} = \<r>" using in_deg_r in_degree_eq_card_arc_blocks
    using assms by auto
  thus ?thesis
