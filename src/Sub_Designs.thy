theory Sub_Designs imports Design_Basics
begin
...
sublocale sub_des: design \<U> \<A>
  using des.finite_sets finite_subset points_subset sub_non_empty_blocks by (unfold_locales) (auto)
...
lemma sub_is_proper: "\<U> \<noteq> \<V> \<Longrightarrow> proper_sub_set_system \<U> \<A> \<V> \<B>"
  by (simp add: blocks_subset incidence_system_axioms points_subset proper_sub_set_system.intro proper_sub_set_system_axioms_def psubsetI)
...
lemma ss_proper_implies_sub [intro]: "proper_sub_set_system \<U> \<A> \<V> \<B> \<Longrightarrow> sub_set_system \<U> \<A> \<V> \<B>"
  by (metis proper_sub_set_system.axioms(1) proper_sub_set_system.blocks_subset proper_sub_set_system.points_psubset psubsetE sub_set_system.intro sub_set_system_axioms_def)
...
lemma sub_ssI [intro!]: "incidence_system \<V> \<B> \<Longrightarrow> \<U> \<subseteq> \<V> \<Longrightarrow> \<A> \<subseteq># \<B> \<Longrightarrow> sub_set_system \<U> \<A> \<V> \<B>"
  using incidence_system_def subset_iff by (unfold_locales) (simp_all add: incidence_system.wellformed)
...
lemma combine_sub_sys: "incidence_system \<U> \<A> \<Longrightarrow> sub_incidence_system \<V> \<B> (\<V> \<union> \<U>) (\<B> + \<A>)"
  using combine_incidence_systems incidence_system_def wellformed by (unfold_locales, auto)

lemma add_point_sub_sys: "sub_incidence_system \<V> \<B> (points (add_point p)) (blocks (add_point p))"
  using add_point_wf by (simp add: sub_ssI subset_insertI incidence_system_axioms sub_incidence_system.intro) 

lemma strong_del_point_sub_sys: "sub_incidence_system (points (strong_delete_point p)) (blocks (strong_delete_point p)) \<V> \<B> "
  using strong_del_point_incidence_wf wf_invalid_point by (unfold_locales) (auto)

lemma add_block_sub_sys: "sub_incidence_system \<V> \<B> (points (add_block b)) (blocks (add_block b))"
  using add_block_wf wf_invalid_point by (unfold_locales) (auto)

lemma delete_block_sub_sys: "sub_incidence_system (points (delete_block b)) (blocks (delete_block b))  \<V> \<B> "
  using delete_block_wf delete_block_blocks_subset incidence_system_def by (unfold_locales, auto)
...
lemma combine_sub_des: assumes "design \<U> \<A>"
  shows "sub_design \<V> \<B> (\<V> \<union> \<U>) (\<B> + \<A>)"
  using combine_designs assms combine_sub_sys
  by (simp add: design.axioms(1) finite_incidence_system.axioms(1) design.sub_designI wf_design) 

lemma add_point_sub_des: "sub_design \<V> \<B> (points (add_point p)) (blocks (add_point p))"
  using add_point_design add_point_sub_sys sub_design.intro by fastforce 

lemma strong_del_point_sub_des: "sub_design (points (strong_delete_point p)) (blocks (strong_delete_point p)) \<V> \<B> "
  using strong_del_point_sub_sys sub_design.intro wf_design by blast 

lemma add_block_sub_des: "finite b \<Longrightarrow> b \<noteq> {} \<Longrightarrow> sub_design \<V> \<B> (points (add_block b)) (blocks (add_block b))"
  using add_block_sub_sys add_block_design sub_designII by fastforce

lemma delete_block_sub_des: "sub_design (points (delete_block b)) (blocks (delete_block b)) \<V> \<B> "
