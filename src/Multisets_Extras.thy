theory Multisets_Extras imports Main "HOL-Library.Multiset" "Nested_Multisets_Ordinals.Multiset_More" "HOL-Library.Disjoint_Sets"
"Card_Partitions.Set_Partition"
begin

section \<open>Micellanious Helper Functions Sets and Multisets\<close>

subsection \<open>Set Theory Extras\<close>

text \<open>A number of extra helper lemmas for reasoning on sets (finite) required for Design Theory proofs \<close>

lemma card_Pow_filter_one: assumes "finite A" shows "card {x \<in> Pow A . card x = 1}  = card (A)"
  using assms 
...
next
  ...
  then have split: "{y \<in> Pow (insert x F) . card y = 1} = {y \<in> (Pow F) . card y = 1} \<union> {y \<in> (insert x ` Pow F) . card y = 1}"
    by blast 
  ...
  then have "card {y \<in> (insert x ` Pow F) . card y = 1} = 1"
    by (metis (full_types, lifting) Collect_empty_eq_bot Pow_bottom bot_empty_eq empty_iff imageI is_singletonI is_singletonI' is_singleton_altdef mem_Collect_eq) (* LONG *) 
  then have " {y \<in> (insert x ` Pow F) . card y = 1} = {{x}}"
    by (smt single card_1_singletonE card_eq_0_iff empty_Collect_eq mem_Collect_eq singletonD zero_neq_one) 
  then have split2:"{y \<in> Pow (insert x F) . card y = 1} = {y \<in> (Pow F) . card y = 1} \<union> {{x}}" using split by simp
  then show ?case 
  proof (cases "x \<in> F")
    case True
      then show ?thesis
  ...
  next
    ...
    then have fact:"card {y \<in> Pow (insert x F) . card y = 1} = card {y \<in> (Pow F) . card y = 1} + card {{x}}" using split2 card_Un_disjoint
      using insert.hyps(1) by auto 
    have "card (insert x F) = card F + 1" using False card_insert_disjoint
      by (metis Suc_eq_plus1 insert.hyps(1)) 
    then show ?thesis using fact
...
lemma set_card_diff_ge_zero: "finite A \<Longrightarrow> finite B \<Longrightarrow> A \<noteq> B \<Longrightarrow> card A = card B \<Longrightarrow> card (A - B) > 0"
...
proof (intro partition_onI) 
  ...
  proof -
    ...
    obtain p1' p2' where orig1: "p1 = p1' - {x}" and orig2: "p2 = p2' - {x}" 
       and origne: "p1' \<noteq> p2'" and ne1: "p1' \<noteq> {x}" and ne2:"p2' \<noteq> {x}" and ing1: "p1' \<in> G" and ing2: "p2' \<in> G"
...
lemma multiset_add_filter_size: "size {# a \<in># (A1 + A2) . P a #} = size {# a \<in># A1 . P a #} + size {# a \<in># A2 . P a #}" 
  by simp

lemma size_filter_neg: "size {#a \<in># A . P a #} = size A - size {# a \<in># A . \<not> P a #}"
  by (metis ordered_cancel_comm_monoid_diff_class.le_imp_diff_is_add size_filter_mset_lesseq size_union union_filter_mset_complement) 
...
proof -
  have "filter_mset P (filter_mset Q A) = filter_mset (\<lambda> a. Q a \<and> P a) A" by (simp add: filter_filter_mset)
  thus ?thesis using assms
    by (metis (mono_tags, lifting) filter_mset_cong)
qed

lemma filter_filter_mset_ss_member: "filter_mset (\<lambda> a . {x, y} \<subseteq> a) A = filter_mset (\<lambda> a . {x, y} \<subseteq> a) (filter_mset (\<lambda> a . x \<in> a) A)"
proof - 
  have filter: "filter_mset (\<lambda> a . {x, y} \<subseteq> a) (filter_mset (\<lambda> a . x \<in> a) A) = filter_mset (\<lambda> a . x \<in> a \<and> {x, y} \<subseteq> a) A" by (simp add: filter_filter_mset)
...
lemma image_image_mset: "image_mset (\<lambda> x . f x) (image_mset (\<lambda> y . g y) A) = image_mset (\<lambda> x. f (g x)) A"
  by simp
...
proof (rule allI)
  fix x
  have count_mult: "count (A \<times># B) x = count A (fst x) * count B (snd x)" using count_Sigma_mset
...
lemma size_union_distinct_cart_prod: "x1 \<noteq> x2 \<Longrightarrow> size (({#x1#} \<times># A) \<union># ({#x2#} \<times># B)) = size ({#x1#} \<times># A) + size ({#x2#} \<times># B)"
  by (simp add: cart_product_single_intersect size_Un_disjoint) 

lemma size_Union_distinct_cart_prod: "distinct_mset M \<Longrightarrow> size (\<Sum>p\<in>#M. ({#p#} \<times># B)) = size (M) * size (B)"
  by (induction M) auto

lemma size_Union_distinct_cart_prod_filter: "distinct_mset M \<Longrightarrow> (\<And> p . p \<in># M \<Longrightarrow> size ({# b \<in># B . P p b #}) = c) \<Longrightarrow> size (\<Sum>p\<in>#M. ({#p#} \<times># {# b \<in># B . P p b #})) = size (M) * c"
  by (induction M) auto

lemma size_Union_distinct_cart_prod_filter2: "distinct_mset V \<Longrightarrow> (\<And> b . b \<in># B \<Longrightarrow> size ({# v \<in># V . P v b #}) = c) \<Longrightarrow> size (\<Sum>b\<in>#B. ( {# v \<in># V . P v b #} \<times># {#b#})) = size (B) * c"
  by (induction B) auto
...
lemma cart_product_add_1_filter: "{#m \<in># ((add_mset a M) \<times># N) . P m #} = {#m \<in># (M \<times># N) . P m #} + {#m \<in># ({#a#} \<times>#  N) . P m #}"
  unfolding add_mset_add_single [of a M] Sigma_mset_plus_distrib1
  by (simp add: Times_mset_single_left)

lemma cart_product_add_1_filter2: "{#m \<in># (M \<times># (add_mset b N)) . P m #} = {#m \<in># (M \<times># N) . P m #} + {#m \<in># (M \<times>#  {#b#}) . P m #}"
...
lemma cart_product_add_1_filter_eq_mirror: "{#m \<in># M \<times># (add_mset b N) . (fst m \<in> snd m) #} = {#m \<in># (M \<times># N) . (fst m \<in> snd m) #} + ({# n \<in># M . n \<in> b #} \<times># {#b#})"
  unfolding add_mset_add_single [of b N] Sigma_mset_plus_distrib1
  by (metis (no_types) add_mset_add_single cart_product_add_1_filter2 cart_product_singleton_right) (* longish *)
...
lemma sum_over_fun_eq: assumes "\<And> x . x \<in># A \<Longrightarrow> f x = g x" shows "(\<Sum>x \<in># A . f(x)) = (\<Sum> x \<in># A . g (x))"
  using assms by auto
...
lemma mset_set_size_card_count:
...
next
  ...
  have rhs: "(\<Sum>x\<in>set_mset (add_mset y A). x * count (add_mset y A) x) = (\<Sum>x\<in>(insert y (set_mset A)) . x * count (add_mset y A) x)"
    by simp 
  then show ?case 
  proof (cases "y \<in># A")
    case True
    have x_val: "\<And> x . x \<in> (insert y (set_mset A)) \<Longrightarrow> x \<noteq> y \<Longrightarrow> x* count (add_mset y A) x = x * (count A x)" 
      by auto 
    have y_count: "count (add_mset y A) y = 1 + count A y" using True
      using count_inI by fastforce
    then have "(\<Sum>x\<in>set_mset (add_mset y A). x * count (add_mset y A) x) = (y * (count (add_mset y A) y)) + (\<Sum>x\<in>(set_mset A) - {y}. x * count A x)" 
    ...
    then have s1: "(\<Sum>x\<in>set_mset (add_mset y A). x * count (add_mset y A) x) = y + y * (count A y) + (\<Sum>x\<in>(set_mset A) - {y}. x * count A x)" 
      using y_count by simp
    then have "(\<Sum>x\<in>set_mset (add_mset y A). x * count (add_mset y A) x) = y + (\<Sum>x\<in>insert y ((set_mset A) - {y} ) . x * count A x)" 
      by (simp add: sum.insert_remove) 
    then have "(\<Sum>x\<in>set_mset (add_mset y A). x * count (add_mset y A) x) = y + (\<Sum>x\<in>(set_mset A) . x * count A x)"
      by (simp add:  True insert_absorb)
    then show ?thesis using lhs add.IH
      by linarith 
  next
    case False
    have x_val: "\<And> x . x \<in> set_mset A \<Longrightarrow> x* count (add_mset y A) x = x * (count A x)" using False
      by auto 
    have y_count: "count (add_mset y A) y = 1" using False count_inI by fastforce
    have lhs: "(\<Sum>x\<in>#add_mset y A. x) = (\<Sum>x\<in># A. x) + y" by simp
    have "(\<Sum>x\<in>set_mset (add_mset y A). x * count (add_mset y A) x) = (y * count (add_mset y A) y) + (\<Sum>x\<in>set_mset A. x * count A x)" 
      using x_val rhs by (metis (no_types, lifting) False finite_set_mset sum.cong sum.insert) 
    then have "(\<Sum>x\<in>set_mset (add_mset y A). x * count (add_mset y A) x) = y + (\<Sum>x\<in>set_mset A. x * count A x)" 
      using y_count by simp 
    then show ?thesis using lhs
      using add.IH by linarith 
  qed
qed
...
distinct due to the definition of addition on multisets\<close>

lemma mset_size_partition_dep: "size {# a \<in># A . P a \<or> Q a #} = size {# a \<in># A . P a #} +  size {# a \<in># A . Q a #} -  size {# a \<in># A . P a \<and> Q a #}"
...
lemma partition_on_mset_count_eq: "partition_on_mset A P \<Longrightarrow> a \<in># A \<Longrightarrow> (\<Sum>x \<in># P. count x a) = count A a"
...
proof (rule ccontr)
  ...
  thus False using assms partition_on_mset_count_eq cge elem_in
    by (metis (mono_tags) count_inI local.count multiset_nonemptyE not_numeral_le_zero numeral_One numeral_le_iff partition_on_mset_def semiring_norm(69)) 
qed
...
lemma partition_on_mset_distinct_disjoint: 
  ...
  assumes "p2 \<in># P - {#p1#}"
  shows "p1 \<inter># p2 = {#}"
  by (smt Diff_eq_empty_iff_mset assms(1) assms(2) assms(3) assms(4) diff_add_zero distinct_mset_add multiset_inter_assoc partition_on_msetD1 subset_mset.inf.absorb_iff2 subset_mset.le_add_same_cancel1 subset_mset.le_iff_inf sum_mset.remove)

lemma partition_on_mset_diff: 
  assumes "partition_on_mset A P"
  assumes "Q \<subseteq>#P"
  shows "partition_on_mset (A - \<Sum>\<^sub>#Q) (P - Q)"
  by (smt assms(1) assms(2) diff_union_cancelL partition_on_mset_def subset_mset.add_diff_inverse sum_mset.union union_iff)
...
lemma partition_on_mset_set: 
  ...
  oops
(*
proof (rule partition_on_msetI)
  have "\<Union>P = A" using assms partition_onD1 by auto 
  have "\<And> x y . (x \<in> (mset_set ` P) \<Longrightarrow> y \<in> (mset_set ` P) \<Longrightarrow> x \<noteq> y \<Longrightarrow> x \<inter># y = {#})"
    using assms oops
  have "\<Sum>\<^sub># (mset_set (mset_set ` P)) = mset_set A"  *)

lemma partition_on_set_mset:
  assumes "partition_on_mset A P"
  assumes "distinct_mset A"
  shows "partition_on (set_mset A) (set_mset (image_mset (\<lambda> x. set_mset x) P))"
  oops
...
lemma partition_on_mset_add:
  ...
  by (smt add_mset_add_single assms empty_not_add_mset mset_subset_eq_single partition_on_mset_all partition_on_mset_def subset_mset.add_diff_inverse sum_mset.add_mset sum_mset.remove union_iff union_mset_add_mset_left)

lemma partition_on_mset_elem_exists_part:
  assumes "partition_on_mset A P"
  assumes "x \<in># A" 
  obtains p where "p \<in># P" and "x \<in># p"
  by (metis assms(1) assms(2) in_Union_mset_iff partition_on_msetD2 partition_on_msetI partition_on_mset_eq_imp_eq_carrier)

lemma partition_on_mset_combine: 
  assumes "partition_on_mset A P"
  assumes "partition_on_mset B Q"
  shows "partition_on_mset (A + B) (P + Q)"
  apply (simp add: partition_on_mset_def)  
