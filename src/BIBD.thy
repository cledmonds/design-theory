...
proof -
  ...
  then have rhsnot0: "\<Lambda> * (\<v> - 1) \<noteq> 0" using 1
    by presburger 
...
proof (rule ccontr)
  ...
  then have a: "\<not> (v - k > k - l)" using kdef ldef vdef
    by (metis block_size_lt_v index_lt_replication less_imp_le_nat of_nat_diff of_nat_less_imp_less rep_value_sym) 
  have lge: "l \<ge> 0" using ldef
    by simp 
...
proof - 
  ...
  then have "(\<Sum>bl \<in># (remove1_mset b1 \<B>). (b1 |\<inter>| bl)^2) 
    ...
    using sum_mset_add_diff[of "(remove1_mset b1 \<B>)" "\<lambda> bl . (b1 |\<inter>| bl)" "\<lambda> bl . (b1 |\<inter>| bl)^2"]  diff_le
    by presburger  
  then have "(\<Sum>bl \<in># (remove1_mset b1 \<B>). (b1 |\<inter>| bl)^2) 
