...
text \<open>We define a group design to have an additional paramater $G$ which is a partition on the point set $V$\<close>

locale group_design = proper_design + (* Not defined in handbook, precursor to GDD without index constraints*)
...
lemma rep_number_point_group_one: assumes "x \<in> \<V>"
...
lemma v_equals_sum_group_sizes: "\<v> = (\<Sum>G \<in> \<G>. card G)"
  using group_partitions groups_disjoint partition_onD1 card_Union_disjoint group_elements_finite by fastforce 
...
proof - 
  ...
  then have "(\<Sum>G' \<in> (\<G> - {G}). card G') \<ge> 1" using min_group_size
    by (smt DiffD1 elem_exists_non_empty_set finite_Diff groups_finite leI less_le_trans less_one sum_eq_0_iff) (* Longish proof *)
...
proof (intro partition_onI, auto)
  ...
  then show False
    using a1 group_size_implies_group_existance by auto 
next
  fix x
  assume "x \<in> \<G>"
...
proof -
  ...
  have "group_sizes = set_mset ({# card G . G \<in># mset_set \<G> #})" using group_sizes_def
    using groups_finite by auto 
  thus ?thesis using size_repr
    by (simp add: count_size_set_repr) 
qed

lemma v_sum_type_rep: "\<v> = (\<Sum> g \<in> group_sizes . g * (groups_of_size g))"
  using group_size_partition_covers_points 
proof -
  have gs: "set_mset {# card G . G \<in># mset_set \<G> #} = group_sizes" unfolding group_sizes_def
    using groups_finite by auto 
  have "\<v> = card (\<Union>(\<Union>{{ G \<in> \<G> . card G = g } | g . g \<in> group_sizes}))" using group_size_partition_covers_points by simp
  ...
  thus ?thesis using gs groups_of_size_alt_def_count
    by auto 
qed
...
proof -
  obtain G where "G \<in> \<G>" using groups_size
    using elem_exists_non_empty_set gr_implies_not_zero by blast 
  thus ?thesis using uniform_groups min_group_size
    by fastforce
qed
...
lemma uniform_groups_group_sizes: "group_sizes = {\<m>}"
  using design_points_nempty group_card_in_sizes group_size_implies_group_existance point_has_unique_group uniform_groups_alt by force
...
Each pair of elements must occur either \Lambda times if in diff groups, or 0 times if in the same group \<close>

locale GDD = group_design + 
  fixes index :: int ("\<Lambda>")
  assumes index_ge_1: "\<Lambda> \<ge> 1"
  assumes index_together: "G \<in> \<G> \<Longrightarrow> x \<in> G \<Longrightarrow> y \<in> G \<Longrightarrow> x \<noteq> y \<Longrightarrow> points_index \<B> {x, y} = 0"
  assumes index_distinct: "G1 \<in> \<G> \<Longrightarrow> G2 \<in> \<G> \<Longrightarrow> G1 \<noteq> G2 \<Longrightarrow> x \<in> G1 \<Longrightarrow> y \<in> G2 \<Longrightarrow> points_index \<B> {x, y} = \<Lambda>"
begin
...
lemma index_together_alt_ss: "ps \<subseteq> G \<Longrightarrow> G \<in> \<G> \<Longrightarrow> card ps = 2 \<Longrightarrow>  points_index \<B> ps = 0"
  using index_together by (metis card_2_iff insert_subset) 

lemma index_distinct_alt_ss: "ps \<subseteq> \<V> \<Longrightarrow> card ps = 2 \<Longrightarrow> (\<And> G . G \<in> \<G> \<Longrightarrow> \<not> ps \<subseteq> G) \<Longrightarrow> points_index \<B> ps = \<Lambda>"
  using index_distinct
  by (metis card_2_iff empty_subsetI insert_subset point_has_unique_group) 

lemma gdd_index_options: "ps \<subseteq> \<V> \<Longrightarrow> card ps = 2 \<Longrightarrow> points_index \<B> ps = 0 \<or> points_index \<B> ps = \<Lambda>"
  using index_distinct_alt_ss index_together_alt_ss by blast

lemma index_zero_implies_same_group: "ps \<subseteq> \<V> \<Longrightarrow> card ps = 2 \<Longrightarrow> points_index \<B> ps = 0 \<Longrightarrow> \<exists> G \<in> \<G> . ps \<subseteq> G"
  by (metis gr_implies_not_zero index_distinct_alt_ss index_ge_1 less_one of_nat_0 of_nat_1 of_nat_le_0_iff)

lemma index_zero_implies_same_group_unique: "ps \<subseteq> \<V> \<Longrightarrow> card ps = 2 \<Longrightarrow>  points_index \<B> ps = 0 \<Longrightarrow> \<exists>! G \<in> \<G> . ps \<subseteq> G"
  by (meson GDD.index_zero_implies_same_group GDD_axioms card_2_iff' group_design.point_in_one_group group_design_axioms in_mono)

lemma index_not_zero_impl_diff_group: "ps \<subseteq> \<V> \<Longrightarrow> card ps = 2 \<Longrightarrow> points_index \<B> ps = \<Lambda> \<Longrightarrow>  (\<And> G . G \<in> \<G> \<Longrightarrow> \<not> ps \<subseteq> G)"
  using index_ge_1 index_together_alt_ss by auto

lemma index_zero_implies_one_group: assumes "ps \<subseteq> \<V>" and  "card ps = 2" and "points_index \<B> ps = 0" 
  shows "size {#b \<in>#  mset_set \<G> . ps \<subseteq> b#} = 1"
proof -
  obtain G where "G \<in> \<G>" and psin: "ps \<subseteq> G" using index_zero_implies_same_group groups_size assms
    by blast
  then have unique: "\<And> G2 . G2 \<in> \<G> \<Longrightarrow> G \<noteq> G2 \<Longrightarrow> \<not> ps \<subseteq> G2" using index_zero_implies_same_group_unique
  ...
  then have "{b \<in> \<G> . ps \<subseteq> b} = {G}" using unique psin
    by (smt Collect_cong \<open>\<And>thesis. (\<And>G. \<lbrakk>G \<in> \<G>; ps \<subseteq> G\<rbrakk> \<Longrightarrow> thesis) \<Longrightarrow> thesis\<close> singleton_conv)
  thus ?thesis by (simp add: eq_mset) 
qed

lemma index_distinct_group_num_alt_def: "ps \<subseteq> \<V> \<Longrightarrow> card ps = 2 \<Longrightarrow> size {#b \<in>#  mset_set \<G> . ps \<subseteq> b#} = 0 \<Longrightarrow> points_index \<B> ps = \<Lambda>"
  by (metis gdd_index_options index_zero_implies_one_group numeral_One zero_neq_numeral)

lemma index_non_zero_implies_no_group: assumes "ps \<subseteq> \<V>" and  "card ps = 2" and "points_index \<B> ps = \<Lambda>" 
...
lemma gdd_index_non_zero_iff: "ps \<subseteq> \<V> \<Longrightarrow> card ps = 2 \<Longrightarrow> points_index \<B> ps = \<Lambda> \<longleftrightarrow> size {#b \<in>#  mset_set \<G> . ps \<subseteq> b#} = 0"
  using index_non_zero_implies_no_group  index_distinct_group_num_alt_def by auto

lemma gdd_index_zero_iff: "ps \<subseteq> \<V> \<Longrightarrow> card ps = 2 \<Longrightarrow> points_index \<B> ps = 0 \<longleftrightarrow> size {#b \<in>#  mset_set \<G> . ps \<subseteq> b#} = 1"
...
lemma points_index_upper_bound: "ps \<subseteq> \<V> \<Longrightarrow> card ps = 2 \<Longrightarrow> (points_index \<B> ps) \<le> \<Lambda>"
  using gdd_index_options
  by (metis index_ge_1 int_one_le_iff_zero_less le_refl of_nat_0 of_nat_0_le_iff of_nat_le_iff zero_less_imp_eq_int) 
...
proof (rule ccontr)
  ...
  obtain ps where ps: "ps \<subseteq> bl \<and> card ps = 2"
    using nat_int_comparison(3) obtain_subset_with_card_n
    by (metis assms(3))  
  then have "points_index \<B> ps \<ge> 2"
    using m points_index_count_min ps by blast
  then show False using assms index_distinct ps antisym_conv2 not_numeral_less_zero numeral_le_one_iff points_index_ps_nin semiring_norm(69) zero_neq_numeral
...
subsection \<open>GDD's \<close>

text \<open>In literature, a GDD is usually defined to have a set of block sizes, or uniform block size \<close>
...
locale K_GDD = K_\<Lambda>_GDD \<V> \<B> \<K> \<G> 1 for point_set ("\<V>") and block_collection ("\<B>") and sizes ("\<K>") and groups ("\<G>")

locale k_GDD = k_\<Lambda>_GDD \<V> \<B> \<k> \<G> 1 for point_set ("\<V>") and block_collection ("\<B>") and u_block_size ("\<k>") and groups ("\<G>")

sublocale k_GDD \<subseteq> K_GDD \<V> \<B> "{\<k>}" \<G>
  by (unfold_locales)

lemma (in K_GDD) multiplicity_1: 
  assumes "bl \<in># \<B>"
  assumes "card bl \<ge> 2"
  shows "multiplicity bl = 1"
  using index_1_imp_mult_1
  by (simp add: assms(1) assms(2)) 

locale RGDD = GDD + resolvable_design

subsection \<open> GDD and PBD's \<close>
...
shows "points_index (blocks (delete_point x)) {y, z} = 0"
proof -
  ...
  then have "{y, z} \<subseteq> points (delete_point x)"
    using \<open>y \<noteq> x\<close> \<open>z \<noteq> x\<close> by fastforce 
...
lemma delete_point_index: 
  ...
  shows "points_index (blocks (delete_point x)) {y, z} = \<Lambda>"
proof -
  ...
  then have "{y, z} \<subseteq> \<V>"
    by (metis ing1 ing2 ss1 ss2 assms(4) assms(5) empty_subsetI insert_absorb insert_subset point_in_group) 
  then have "{y, z} \<subseteq> points (delete_point x)"
    using \<open>y \<noteq> x\<close> \<open>z \<noteq> x\<close> by auto 
  then have indx: "points_index (blocks (delete_point x)) {y, z} = points_index \<B> {y, z}" 
...
lemma delete_point_group_size: assumes "{x} \<in> \<G> \<Longrightarrow> card \<G> > 2" shows "1 < card {g - {x} |g. g \<in> \<G> \<and> g \<noteq> {x}}"
proof (cases "{x} \<in> \<G>")
  case True
...
next
  case False
  then have inj: "inj_on (\<lambda> g . g - {x}) \<G>" apply (intro inj_onI)
...
lemma GDD_by_deleting_point: 
  ...
  shows "GDD (points (delete_point x)) (blocks (delete_point x)) {g - {x} | g . g \<in> \<G> \<and> g \<noteq> {x}} \<Lambda>"
proof -
  interpret pd: proper_design "points (delete_point x)" "blocks (delete_point x)"
  ...
  show ?thesis using delete_point_index_zero delete_point_index assms delete_point_group_size
    by(unfold_locales, simp_all add: partition_on_remove_pt group_partitions index_ge_1)
qed
...
lemma kgdd1_points_index_group_block: 
  ...
  and "card ps = 2"
  shows "points_index (\<B> + mset_set \<G>) ps = 1"
proof -
  have index1: "(\<And> G . G \<in> \<G> \<Longrightarrow> \<not> ps \<subseteq> G) \<Longrightarrow> points_index \<B> ps = 1"
    using index_distinct_alt_ss assms by fastforce 
  have groups1: "points_index \<B> ps = 0 \<Longrightarrow> size {#b \<in>#  mset_set \<G> . ps \<subseteq> b#} = 1"  
    using index_zero_implies_one_group assms by simp 
  have "points_index (\<B> + mset_set \<G>) ps = size {#b \<in># \<B> + mset_set \<G> . ps \<subseteq> b#}" by (simp add: points_index_def)
  then have " points_index (\<B> + mset_set \<G>) ps = size (filter_mset ((\<subseteq>) ps) (\<B> + mset_set \<G>))" by simp
  thus ?thesis using index1 groups1 gdd_index_non_zero_iff gdd_index_zero_iff assms 
    by (smt (z3) empty_neutral(1) filter_union_mset gdd_index_options less_irrefl_nat nonempty_has_size of_nat_1_eq_iff points_index_def union_commute) 
qed

lemma combine_block_groups_pairwise:
  shows "pairwise_balance \<V> (\<B> + mset_set \<G>) 1"
proof -
  let ?B = "\<B> + mset_set \<G>"
  have "\<And> G. G \<in> \<G> \<Longrightarrow> G \<subseteq> \<V>"
  ...
  then interpret inc: design \<V> ?B
    using  finite_sets groups_finite incidence_system.finite_sysI incidence_system_def point_in_group subsetI wellformed
    by (smt (verit) blocks_nempty_alt design_blocks_nempty design_points_nempty empty_eq_union finite_set_mset_mset_set fst_conv snd_conv union_iff wf_design_implies)
  show ?thesis proof (unfold_locales, simp_all)
    show "\<b> + int (card \<G>) \<noteq> 0" using b_positive by auto
    show "2 \<le> inc.\<v>" by (simp add: gdd_min_v)
    then show "\<And>ps. ps \<subseteq> \<V> \<Longrightarrow> card ps = 2 \<Longrightarrow> points_index (\<B> + mset_set \<G>) ps = Suc 0" 
...
lemma kgdd1_index_adjoin_group_block:
  ...
  assumes "card ps = 2"
  shows "points_index (\<B> + mset_set {insert x g |g. g \<in> \<G>}) ps = 1"
proof -
  ...
  proof (cases "x \<in> ps")
    ...
    have b: "points_index \<B> ps = 0"
      using True assms(1) points_index_ps_nin by fastforce 
    then have "points_index (\<B> + mset_set {insert x g |g. g \<in> \<G>}) ps = points_index (mset_set {insert x g |g. g \<in> \<G>}) ps"
    ...
    also have 0: "... = card {b \<in> {insert x g |g. g \<in> \<G>} . y \<in> b}" by (simp add: groups_finite rep_number_on_set_def)
    also have 1: "... = card {insert x g |g. g \<in> \<G> \<and> y \<in> insert x g}"
      by (smt (verit) Collect_cong mem_Collect_eq) (* FASTER WITH FILTER REF?)*)
    also have 2: " ... =card {insert x g |g. g \<in> \<G> \<and> y \<in> g}" using iff
      by metis 
    also have "... = card {g \<in> \<G> . y \<in> g}" (*SLOW *)
      by (metis 1 2 0 empty_iff eq groups_finite insert_iff points_index_block_image_add_eq points_index_single_rep_num rep_number_on_set_def ynex)  
    finally have "points_index (\<B> + mset_set {insert x g |g. g \<in> \<G>}) ps = 1" using rep_number_point_group_one yinv
      by simp 
    then show ?thesis
      by simp 
  next
    ...
    then have "points_index (\<B> + mset_set {insert x g |g. g \<in> \<G>}) ps = points_index (\<B> + mset_set \<G>) ps"
      using eq by (simp add: point_index_distrib points_index_block_image_add_eq False) 
...
lemma pairwise_by_adjoining_point: 
  assumes "x \<notin> \<V>"
  shows "pairwise_balance (points (add_point x)) (\<B> + mset_set { insert x g | g. g \<in> \<G>}) 1"
proof -
  let ?B = "\<B> + mset_set { insert x g | g. g \<in> \<G>}"
  let ?V = "points (add_point x)"
  have "?V = \<V> \<union> {x}" by simp
  show ?thesis using finite_sets b_positive proof (unfold_locales, simp_all)
    ...
    then show "\<And>b. b \<in># \<B> \<or> b \<in># mset_set {insert x g |g. g \<in> \<G>} \<Longrightarrow> b \<subseteq> insert x \<V>" using wellformed by fastforce
  next 
    ...
    then have gnempty: "\<And> G. G \<in># (mset_set { insert x g | g. g \<in> \<G>}) \<Longrightarrow> G \<noteq> {}"
      by (smt (verit, del_insts) elem_mset_set empty_iff infinite_set_mset_mset_set mem_Collect_eq)
    then show "\<And>bl. bl \<in># \<B> \<or> bl \<in># mset_set {insert x g |g. g \<in> \<G>} \<Longrightarrow> bl \<noteq> {}" using blocks_nempty by auto
  next
  ...
  next
    show "\<And>ps. ps \<subseteq> insert x \<V> \<Longrightarrow>
          card ps = 2 \<Longrightarrow> points_index (\<B> + mset_set {insert x g |g. g \<in> \<G>}) ps = Suc 0" 
...
lemma PBD_by_adjoining_point: 
  assumes "x \<notin> \<V>"
  assumes "\<And> k . k \<in> \<K> \<Longrightarrow> k \<ge> 2"
  shows "PBD (points (add_point x)) (\<B> + mset_set { insert x g | g. g \<in> \<G>}) (\<K> \<union> {(card g) + 1 | g . g \<in> \<G>})"
proof -
  let ?B = "\<B> + mset_set { insert x g | g. g \<in> \<G>}"
  let ?V = "points (add_point x)"
  ...
  show ?thesis using  block_sizes positive_ints proof (unfold_locales)
    ...
    have "\<And> bl . bl \<in># mset_set {insert x g |g. g \<in> \<G>} \<Longrightarrow> bl \<in> {insert x g | g . g \<in> \<G>}"
      by (simp add: groups_finite) 
    then have "\<And> bl . bl \<in># mset_set {insert x g |g. g \<in> \<G>} \<Longrightarrow> card bl \<in>  {int (card g + 1) |g. g \<in> \<G>}" 
    proof -
      fix bl 
    ...
    qed
    then show "\<And>bl. 
          bl \<in># \<B> + mset_set {insert x g |g. g \<in> \<G>} \<Longrightarrow> int (card bl) \<in> \<K> \<union> {int (card g + 1) |g. g \<in> \<G>}"
...
proof -
  interpret bkgdd: K_GDD "(B \<times> I)" "(f B)" \<K>' "{{x} \<times> I |x . x \<in> B }"
    using assms by simp
  show "card b \<in> \<K>'" using bkgdd.block_sizes
      by (simp add:assms) 
  qed
...
lemma wilsons_construction_index_0: 
  ...
  assumes "Y \<in> G" 
  assumes "X \<noteq> Y"
  shows "points_index (\<Sum>\<^sub># (image_mset f \<B>)) {X, Y} = 0"
proof -
  ...
  have ne_index_0: "x \<noteq> y \<Longrightarrow> points_index \<B> {x, y} = 0" using ying xing index_together ging by simp
  have "\<And> B. B \<in># \<B> \<Longrightarrow> points_index (f B) {(x, ix), (y, iy)} = 0" 
  proof -
    fix B
    assume assm: "B \<in># \<B>"
    then interpret kgdd: K_GDD "(B \<times> I)" "(f B)" \<K>' "{{x} \<times> I |x . x \<in> B }" using assms by simp
    have not_ss_0: "\<not> ({(x, ix), (y, iy)} \<subseteq> (B \<times> I)) \<Longrightarrow> points_index (f B) {(x, ix), (y, iy)} = 0"
      by (metis kgdd.points_index_ps_nin) 
    have "x \<noteq> y \<Longrightarrow> \<not> {x, y} \<subseteq> B" using ne_index_0 assm points_index_0_left by auto 
    ...
    then have nexy: "x \<noteq> y \<Longrightarrow> points_index (f B) {(x, ix), (y, iy)} = 0" using not_ss_0 by simp
    have "x = y \<Longrightarrow> ({(x, ix), (y, iy)} \<subseteq> (B \<times> I)) \<Longrightarrow> points_index (f B) {(x, ix), (y, iy)} = 0"
    proof -
      assume eq: "x = y"
    ...
    qed
    then show "points_index (f B) {(x, ix), (y, iy)} = 0" using not_ss_0 nexy by auto
  qed
  then have "\<And> B. B \<in># (image_mset f \<B>) \<Longrightarrow> points_index B {(x, ix), (y, iy)} = 0" by auto
  then show "points_index (\<Sum>\<^sub># (image_mset f \<B>)) {X, Y} = 0" by (simp add: points_index_sum xpair ypair)
qed

lemma wilsons_construction_index_0v2: 
  assumes "\<And> B . B \<in># \<B> \<Longrightarrow> K_GDD (B \<times> I) (f B) \<K>' {{x} \<times> I |x . x \<in> B }"
  shows "G \<in> {GG \<times> I |GG. GG \<in> \<G>} \<Longrightarrow> X \<in> G \<Longrightarrow> Y \<in> G \<Longrightarrow> X \<noteq> Y \<Longrightarrow> points_index (\<Sum>\<^sub># (image_mset f \<B>)) {X, Y} = 0"
  using local.wilsons_construction_index_0[OF  assms] by auto
...
lemma wilsons_construction_index_1: 
  ...
  shows  "points_index (\<Sum>\<^sub># (image_mset f \<B>)) {(x, ix), (y, iy)} = 1"
proof -
  ...
  then have pi0: "\<And> b . b \<in># \<B> - {#bl#} \<Longrightarrow> points_index (f b) {(x, ix), (y, iy)}  = 0"
  proof -
    fix b
    assume assm: "b \<in># \<B> - {#bl#}"
    then have "b \<in># \<B>" by (meson in_diffD) 
    then interpret kgdd: K_GDD "(b \<times> I)" "(f b)" \<K>' "{{x} \<times> I |x . x \<in> b }" using assms by simp
    show "points_index (f b) {(x, ix), (y, iy)} = 0"
  ...
  obtain g1 g2 where xing1: "(x, ix) \<in> g1" and ying2: "(y, iy) \<in> g2" and g1g: "g1 \<in> ?G" and g2g: "g2 \<in> ?G"
    by (metis (no_types, lifting) assms(8) assms(9) bkgdd.point_has_unique_group gi1 gi2 insert_subset mem_Sigma_iff xyblss) 
  then have "g1 \<noteq> g2" using xyne by blast 
  then have pi1: "points_index (f bl) {(x, ix), (y, iy)} = 1" using bkgdd.index_distinct xing1 ying2 g1g g2g by simp
  have "points_index (\<Sum>\<^sub># (image_mset f \<B>)) {(x, ix), (y, iy)} = (\<Sum>B \<in># \<B>. points_index (f B) {(x, ix), (y, iy)} )" 
    by (simp add: points_index_sum)
  then have "points_index (\<Sum>\<^sub># (image_mset f \<B>)) {(x, ix), (y, iy)} = (\<Sum>B \<in># (\<B> - {#bl#}). points_index (f B) {(x, ix), (y, iy)}) + points_index (f bl) {(x, ix), (y, iy)}"
...
proof -
  ...
  then interpret kdes: K_block_design ?Y ?B \<K>' using assms(3) by (unfold_locales) (simp_all,fastforce)
  interpret gdd: GDD ?Y ?B ?H "1:: int" 
  proof (unfold_locales)
    show "partition_on (\<V> \<times> I) {G \<times> I |G. G \<in> \<G>}" using assms groups_not_empty design_points_nempty group_partitions
    ...
    have gdd_fact: "\<And> B . B \<in># \<B> \<Longrightarrow> K_GDD (B \<times> I) (f B) \<K>' {{x} \<times> I |x . x \<in> B }" using assms by simp
    show "\<And>G X Y. G \<in> {GG \<times> I |GG. GG \<in> \<G>} \<Longrightarrow> X \<in> G \<Longrightarrow> Y \<in> G \<Longrightarrow> X \<noteq> Y \<Longrightarrow> points_index (\<Sum>\<^sub># (image_mset f \<B>)) {X, Y} = 0"
    proof - (* Why copy in? *)
      fix G X Y
      assume a1: "G \<in> {GG \<times> I |GG. GG \<in> \<G>}" 
      assume a2: "X \<in> G" and a3: "Y \<in> G" and a4: "X \<noteq> Y"
      obtain G' where gi: "G = G' \<times> I" and ging: "G' \<in> \<G>" using a1 by auto
      obtain x y ix iy where xpair: "X = (x, ix)" and ypair: "Y = (y, iy)" using assms a2 a3 gi by auto
      then have ixin: "ix \<in> I" and xing: "x \<in> G'" using a2 gi by auto 
      have iyin: "iy \<in> I" and ying: "y \<in> G'" using a3 ypair gi by auto
      have ne_index_0: "x \<noteq> y \<Longrightarrow> points_index \<B> {x, y} = 0" using ying xing index_together ging by simp
      have "\<And> B. B \<in># \<B> \<Longrightarrow> points_index (f B) {(x, ix), (y, iy)} = 0" 
      proof -
        fix B
        ...
        proof -
          ...
          then obtain g where "g \<in> {{x} \<times> I |x . x \<in> B }" and "(x, ix) \<in> g" and "(y, ix) \<in> g"
            using eq  by auto 
          then show ?thesis using kgdd.index_together
            by (smt (verit, best) SigmaD1 SigmaD2 SigmaI a3 a4 gi mem_Collect_eq xpair ypair)
        qed
        then show "points_index (f B) {(x, ix), (y, iy)} = 0" using not_ss_0 nexy by auto
      qed
      then have "\<And> B. B \<in># (image_mset f \<B>) \<Longrightarrow> points_index B {(x, ix), (y, iy)} = 0" by auto
    ...
    qed
    show "\<And>G1 G2 X Y.
       G1 \<in> {G \<times> I |G. G \<in> \<G>} \<Longrightarrow>
       G2 \<in> {G \<times> I |G. G \<in> \<G>} \<Longrightarrow> G1 \<noteq> G2 \<Longrightarrow> X \<in> G1 \<Longrightarrow> Y \<in> G2 \<Longrightarrow> int (points_index (\<Sum>\<^sub># (image_mset f \<B>)) {X, Y}) = 1"
      using wilsons_construction_index_1 assms 
    proof -
      fix G1 G2 X Y
      assume a0: "G1 \<in> {G \<times> I |G. G \<in> \<G>}" 
      assume a1: "G2 \<in> {G \<times> I |G. G \<in> \<G>}"
      assume a2: "X \<in> G1" and a3: "Y \<in> G2" and a4: "G1 \<noteq> G2"
      obtain G1' where gi1: "G1 = G1' \<times> I" and ging1: "G1' \<in> \<G>" using a0 by auto
      obtain G2' where gi2: "G2 = G2' \<times> I" and ging2: "G2' \<in> \<G>" using a1 by auto
      obtain x y ix iy where xpair: "X = (x, ix)" and ypair: "Y = (y, iy)" using assms a2 a3 gi1 gi2 by auto
      have xing: "x \<in> G1'" using a2 gi1 xpair by simp
      have ying: "y \<in> G2'" using a3 gi2 ypair by simp
      have gne: "G1' \<noteq> G2'" using a4 gi1 gi2 by auto
      then have xyne: "x \<noteq> y" using xing ying ging1 ging2 point_in_one_group by blast
      have "\<exists>! bl . bl \<in># \<B> \<and> {x, y} \<subseteq> bl" using index_distinct points_index_one_unique_block
        by (metis ging1 ging2 gne of_nat_1_eq_iff xing ying) 
      then obtain bl where blinb:"bl \<in># \<B>" and xyblss: "{x, y} \<subseteq> bl" by auto 
      ...
      interpret bkgdd: K_GDD "(bl \<times> I)" "(f bl)" \<K>' ?G using assms blinb by simp
      obtain g1 g2 where xing1: "(x, ix) \<in> g1" and ying2: "(y, iy) \<in> g2" and g1g: "g1 \<in> ?G" and g2g: "g2 \<in> ?G"
       using bkgdd.point_has_unique_group gi1 gi2 insert_subset mem_Sigma_iff xyblss
       by (metis (no_types, lifting) a2 a3 xpair ypair)  
      then have "g1 \<noteq> g2" using xyne by blast 
      then have pi1: "points_index (f bl) {(x, ix), (y, iy)} = 1" using bkgdd.index_distinct xing1 ying2 g1g g2g by simp
      have "points_index (\<Sum>\<^sub># (image_mset f \<B>)) {(x, ix), (y, iy)} = (\<Sum>B \<in># \<B>. points_index (f B) {(x, ix), (y, iy)} )" 
        by (simp add: points_index_sum)
      then have "points_index (\<Sum>\<^sub># (image_mset f \<B>)) {(x, ix), (y, iy)} = (\<Sum>B \<in># (\<B> - {#bl#}). points_index (f B) {(x, ix), (y, iy)}) + points_index (f bl) {(x, ix), (y, iy)}"
        by (metis (no_types, lifting) add.commute blinb insert_DiffM sum_mset.insert) 
      thus "int (points_index (\<Sum>\<^sub># (image_mset f \<B>)) {X, Y}) = 1"  using pi0 pi1 xpair ypair by simp
    qed
  qed
...
lemma PBD_by_deleting_point: 
  ...
  shows "pairwise_balance (points (delete_point x)) (blocks (delete_point x)) \<Lambda>"
proof (cases "x \<in> \<V>")
  case True
  interpret des: design "points (delete_point x)" "blocks (delete_point x)"
    using delete_point_design assms by blast 
  show ?thesis using assms design_blocks_nempty 
  proof (unfold_locales, simp_all)
    show "2 < \<v> \<Longrightarrow> (\<And>bl. bl \<in># \<B> \<Longrightarrow> 2 \<le> card bl) \<Longrightarrow> 2 \<le> int (card (\<V> - {x}))"
      by (metis card_Diff_singleton_if diff_diff_cancel diff_le_mono2 finite_sets int_ops(3) less_nat_zero_code less_one nat_le_linear verit_comp_simplify1(3) zle_int)
    have "\<And> ps . ps  \<subseteq> \<V> - {x} \<Longrightarrow> ps \<subseteq> \<V>" by auto
    then show "\<And>ps. ps \<subseteq> \<V> - {x} \<Longrightarrow> card ps = 2  \<Longrightarrow> (points_index {#bl - {x}. bl \<in># \<B>#} ps) = \<Lambda>"
      using delete_point_index_eq by simp
  qed
next
  case False
  then show ?thesis
    by (simp add: delete_invalid_point_eq pairwise_balance_axioms) 
qed
end
...
lemma bibd_from_kGDD:
  ...
  assumes " x \<notin> \<V>"
  shows "bibd (points (add_point x)) (\<B> + mset_set { insert x g | g. g \<in> \<G>}) (\<k>) 1"
proof - 
  have "({\<k>} \<union> {(card g) + 1 | g . g \<in> \<G>}) = {\<k>}"
    using assms(2) by fastforce 
  then interpret pbd: PBD "points (add_point x)" "\<B> + mset_set { insert x g | g. g \<in> \<G>}" "{\<k>}"
    using PBD_by_adjoining_point assms
    by (smt (verit, best) Collect_cong sys_block_sizes_obtain_bl sys_block_sizes_uniform uniform_alt_def_all) 
  show ?thesis using assms pbd.block_sizes block_size_lt_v finite_sets by (unfold_locales) (simp_all)
qed
...
lemma pbd_points_index1: "ps \<subseteq> \<V> \<Longrightarrow> card ps = 2 \<Longrightarrow>  points_index \<B> ps = 1"
...
proof (rule ccontr)
  ...
  then have ss: "{#b1, b2#} \<subseteq># ?ps"
    by (metis assms(3) insert_noteq_member mset_add mset_subset_eq_add_mset_cancel single_subset_iff) 
  ...
  then have "points_index \<B> {x, y} = 1" using pbd_points_index1 pair by simp
...
lemma strong_delete_point_groups_index_zero: 
  ...
  shows "points_index (blocks (strong_delete_point x)) {xa, y} = 0"
proof (auto simp add: points_index_0_iff)
  ...
  then show False
    by (metis DiffD1 a1 a2 a3 a4 assms(2) assms(3) assms(4) empty_subsetI insert_subset pbd_index1_points_imply_unique_block) 
qed
...
lemma strong_delete_point_groups_index_one: 
  ...
  shows  "points_index (blocks (strong_delete_point x)) {xa, y} = 1"
proof -
  ...
  have "xa \<noteq> y"
    by (smt (verit, best) Diff_eq_empty_iff Diff_iff gb1 b1in xin1 gb2 b2in xin2 assms(3) assms(4) assms(5) empty_Diff insertCI insert_subset pbd_index1_points_imply_unique_block) 
  then have pair: "card {xa, y} = 2"  by simp 
  have inv: "{xa, y} \<subseteq> \<V>"
    by (metis Diff_cancel Diff_subset gb1 b1in gb2 b2in assms(4) assms(5) insert_Diff insert_subset wellformed) 
  have "points_index {# bl \<in># \<B> . x \<in> bl#} {xa, y} = 0"
  proof (auto simp add: points_index_0_iff)
  ...
  qed
  then have "points_index (blocks (strong_delete_point x)) {xa, y} = points_index \<B> {xa, y}" 
    by (metis multiset_partition plus_nat.add_0 point_index_distrib strong_delete_point_blocks) 
...
proof (intro partition_onI )
  ...
  proof (intro subset_antisym subsetI) (* Better rule? *)
    fix xa
  ...
  next 
    ...
    then have "card {b \<in> design_support . {xa, x} \<subseteq> b} = 1" using balanced points_index_simple_def pbd_points_index1 assms
      by (metis pair) 
    ...
    then show "xa \<in> \<Union> {b - {x} |b. b \<in># \<B> \<and> x \<in> b}"
      using des ss nex by auto 
  qed
  show "\<And>p p'. p \<in> {b - {x} |b. b \<in># \<B> \<and> x \<in> b} \<Longrightarrow> p' \<in> {b - {x} |b. b \<in># \<B> \<and> x \<in> b} \<Longrightarrow> p \<noteq> p' \<Longrightarrow> p \<inter> p' = {}" 
  proof -
    fix p p'
    assume p1: "p \<in> {b - {x} |b. b \<in># \<B> \<and> x \<in> b}" and p2: "p' \<in> {b - {x} |b. b \<in># \<B> \<and> x \<in> b}" and pne: "p \<noteq> p'"
    then obtain b where b1: "p = b - {x}" and b1in:"b \<in># \<B>" and xinb1:"x \<in> b" by blast 
    then obtain b' where b2: "p' = b' - {x}" and b2in: "b' \<in># \<B>" and xinb2: "x \<in> b'" using p2 by blast
    then have "b \<noteq> b'" using pne b1 by auto
    then have "\<And> y. y \<in> b \<Longrightarrow> y \<noteq> x \<Longrightarrow> y \<notin> b'" using b1in b2in xinb1 xinb2  pbd_index1_points_imply_unique_block
...
lemma KGDD_by_deleting_point:
  ...
  assumes "\<B> rep x < \<b>"
  assumes "\<B> rep x > 1" 
  shows "K_GDD (points (strong_delete_point x)) (blocks (strong_delete_point x)) \<K> { b - {x} | b . b \<in># \<B> \<and> x \<in> b}"
proof -
  have "\<And> bl. bl \<in># \<B> \<Longrightarrow> card bl \<ge> 2" using block_size_gt_t 
    by (simp add: block_sizes nat_int_comparison(3))
  then interpret des: proper_design "points (strong_delete_point x)" "blocks (strong_delete_point x)" 
    using strong_delete_point_proper assms by blast
  show ?thesis using blocks_with_x_partition strong_delete_point_groups_index_zero strong_delete_point_groups_index_one 
  proof (unfold_locales, simp_all add: block_sizes positive_ints assms ) 
    have ge1: "card {b . b \<in># \<B> \<and> x \<in> b} > 1" using assms(3) replication_num_simple_def by auto
    have fin: "finite {b . b \<in># \<B> \<and> x \<in> b}" by simp 
    have inj: "inj_on (\<lambda> b . b - {x}) {b . b \<in># \<B> \<and> x \<in> b}" using assms(2)
      using inj_on_def mem_Collect_eq by auto 
    then have "card {b - {x} |b. b \<in># \<B> \<and> x \<in> b} = card {b . b \<in># \<B> \<and> x \<in> b}" using card_image fin
      by (simp add: inj card_image setcompr_eq_image)
...
proof (unfold_locales,auto simp add: Setcompr_eq_image partition_on_singletons)
  ...
  show "\<And>xa xb. xa \<in> \<V> \<Longrightarrow> xb \<in> \<V> \<Longrightarrow> points_index \<B> {xa, xb} \<noteq> Suc 0 \<Longrightarrow> xa = xb"
  proof (rule ccontr)
    fix xa xb
    assume ain: "xa \<in> \<V>" and bin: "xb \<in> \<V>" and ne1:"points_index \<B> {xa, xb} \<noteq> Suc 0"
    assume "xa \<noteq> xb"
    then have "card {xa, xb} = 2" by auto
    then have "points_index \<B> {xa, xb} = 1"
      by (simp add: ain bin pbd_points_index1) 
    thus False using ne1
      by linarith
  qed 
qed
...
proof -
  ...
  then interpret kgdd: K_GDD "points (strong_delete_point x)" "blocks (strong_delete_point x)" "{\<k>}" "{ b - {x} | b . b \<in># \<B> \<and> x \<in> b}"
    using pbd.KGDD_by_deleting_point lt assms by blast 
  show ?thesis by (unfold_locales) (simp_all)
qed
